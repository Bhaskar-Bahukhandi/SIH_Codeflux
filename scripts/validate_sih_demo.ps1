...
@(
    ...
    "Production-like Flutter-to-FastAPI integration: PASS (dedicated CI evidence)"
    "Mobile OCR adapter/native build feasibility: PASS (physical runtime still external)"
    "Physical font-size/calibration: DEFERRED BY BLUEPRINT"
    "Hosting/provider decision + container packaging: PASS (live Railway provisioning external)"
    "Live Railway provisioning/persistence: EXTERNAL EVIDENCE REQUIRED"
) | Set-Content -Path $SummaryPath -Encoding UTF8

Write-Host ""
Write-Host "Automated validation completed."
Write-Host "Evidence logs: $LogRoot"
Write-Host "Remaining external/runtime and blueprint-deferred gates are classified in docs/validation/README.md."
